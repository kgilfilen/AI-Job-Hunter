...
if page == "Applications":
    ...
    if not tracked_applications:
        ...
        for tracked_application in tracked_applications:
            job = repository.get_job(tracked_application.job_id)

            if job is None:
                label = f"Job {tracked_application.job_id}"
            else:
                title = (
                    job.get("title")
                    or f"Job {tracked_application.job_id}"
                )
                company = job.get("company")
            ...
            application_labels[tracked_application.id] = label
        ...
        selected_application_id = st.selectbox(
            ...
            key="application_management_selection",
        )
        ...
        if application is not None:
            selected_job = repository.get_job(application.job_id)
            ...
            if application.status == ApplicationStatus.INTERESTED:
                if st.button(
                    "Mark as Applied",
                    key=f"application_mark_applied_{application.id}",
                ):
                ...
                if st.button(
                    "Start Interviewing",
                    key=f"application_interviewing_{application.id}",
                ):
                ...
                if st.button(
                    "Offer Received",
                    key=f"application_offer_{application.id}",
                ):
            ...
            if application.status not in terminal_statuses:
                ...
                rejected_notes = st.text_input(
                    ...
                    key=f"application_rejected_notes_{application.id}",
                )

                if st.button(
                    "Rejected",
                    key=f"application_rejected_{application.id}",
                ):
                ...
                withdrawn_notes = st.text_input(
                    ...
                    key=f"application_withdrawn_notes_{application.id}",
                )

                if st.button(
                    "Withdraw",
                    key=f"application_withdrawn_{application.id}",
                ):
                ...
                close_notes = st.text_input(
                    ...
                    key=f"application_close_notes_{application.id}",
                )

                if st.button(
                    "Close Application",
                    key=f"application_closed_{application.id}",
                ):
            ...
            next_action = st.text_input(
                ...
                key=f"application_next_action_{application.id}",
            )

            follow_up_date = st.date_input(
                "Follow-up date",
                value=None,
                key=f"application_follow_up_date_{application.id}",
            )

            if st.button(
                "Save Follow-up",
                key=f"application_save_follow_up_{application.id}",
            ):
                follow_up_at = None
...
if page == "Jobs":
    ...
    if not stored_jobs:
        ...
        if selected_job is not None:
            ...
            if application is None:
                st.caption("Application tracking has not been started.")
            else:
                st.write(
                    "**Application status:** "
                    f"{application.status.value}"
